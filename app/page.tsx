...
export default function TCSOTerminal() {
  ...
  const fetchPrices = async () => {
    try {
      ...
      if (d && Array.isArray(d)) {
        setPrices({
          BTC: parseFloat(d.find((i) => i.symbol === 'BTCUSDT')?.price || 0).toLocaleString('en-US', {minimumFractionDigits: 1}),
          ETH: parseFloat(d.find((i) => i.symbol === 'ETHUSDT')?.price || 0).toLocaleString('en-US', {minimumFractionDigits: 2}),
          SOL: parseFloat(d.find((i) => i.symbol === 'SOLUSDT')?.price || 0).toLocaleString('en-US', {minimumFractionDigits: 2})
        });
      }
    } catch (e) {}
  };
  ...
  const getTciStatus = (s) => {
    if (s < 45) return { text: "空头主导", color: "text-red-500" };
    if (s <= 55) return { text: "情绪观望", color: "text-amber-400" };
        ...
        <div className="col-span-12 lg:col-span-8 flex flex-col gap-5">
          <div className="bg-[#0B0E14] border border-zinc-800 p-5 relative overflow-hidden flex flex-col md:flex-row justify-between items-center gap-6">
            ...
            <div className="grid grid-cols-2 gap-3 min-w-[240px]">
              <div className="bg-zinc-900/50 border border-zinc-800/80 p-3"><div className="text-[8px] text-zinc-500 uppercase tracking-widest mb-1">ConfidenceLevel</div><div className="text-sm text-zinc-300">{analysis.tci_score === 50 ? "45.0%" : "87.5%"}</div></div>
              <div className="bg-zinc-900/50 border border-zinc-800/80 p-3"><div className="text-[8px] text-zinc-500 uppercase tracking-widest mb-1">VolatilityRate</div><div className="text-sm text-zinc-300">4.2%</div></div>
        ...
        <div className="col-span-12 lg:col-span-4 flex flex-col gap-5">
          <div className="bg-[#0B0E14] border border-zinc-800 p-5">
            ...
            <div className="space-y-4">
              <div><div className="text-[9px] text-emerald-500 uppercase mb-2 font-bold tracking-widest flex items-center gap-1.5"><span className="w-1 h-1 bg-emerald-500 shadow-[0_0_5px_#10b981]"></span> LONG</div>
                <div className="flex flex-wrap gap-1.5">{(analysis.bullish_assets || []).map(a => <span key={a} className="bg-emerald-950/30 text-emerald-400 border border-emerald-900/50 px-2 py-0.5 text-[9px]">{a}</span>)}</div>
              </div>
              <div className="pt-4 border-t border-zinc-800/50"><div className="text-[9px] text-red-500 uppercase mb-2 font-bold tracking-widest flex items-center gap-1.5"><span className="w-1 h-1 bg-red-500 shadow-[0_0_5px_#ef4444]"></span> SHORT</div>
                <div className="flex flex-wrap gap-1.5">{(analysis.bearish_assets || []).map(a => <span key={a} className="bg-red-950/30 text-red-400 border border-red-900/50 px-2 py-0.5 text-[9px]">{a}</span>)}</div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-[#0B0E14] border border-zinc-800 p-5">
            ...
            <div className="space-y-1">
              {['BTC', 'ETH', 'SOL'].map((coin) => (
                <div key={coin} className="flex justify-between items-center py-2 border-b border-zinc-800/50 last:border-0">
                  <span className="text-[11px] font-bold text-zinc-400">{coin}/USDT</span>
                  <span className="text-[13px] font-mono text-zinc-200">{prices[coin]}</span>
                </div>
              ))}
            </div>
          </div>
